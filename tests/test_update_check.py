...
def test_startup_update_notice_reports_newer_stable_release(tmp_path) -> None:
    ...
    assert "Xoder 0.2.0 is available (installed: 0.1.0)" in notice.message
    assert "uv tool upgrade xoder-ai" in notice.message
...
def test_startup_update_notice_uses_xoder_disable_environment_variable(tmp_path) -> None:
    calls: list[tuple[str, float]] = []
...
def test_startup_update_notice_ignores_legacy_tau_disable_environment_variable(tmp_path) -> None:
    notice = startup_update_notice(
        "0.1.0",
        fetcher=lambda _url, _timeout: {"releases": {"0.2.0": [{}]}},
        cache_path=tmp_path / "update-check.json",
...
def test_startup_release_notes_notice_reports_upgrade_once(tmp_path) -> None:
    ...
    assert notice.message == "Xoder updated to 0.1.2\n\n**New**\n- New feature\n- Bug fix"
...
def test_startup_release_notes_notice_survives_missing_release_notes_file(
    ...
    import xoder_coding.update_check as update_check_module
