...
from xoder_coding.tools import _write_temp_output
...
def test_write_temp_output_uses_xoder_prefix_and_preserves_content() -> None:
    output = "complete bash output\n"
    path = Path(_write_temp_output(output))

    try:
        assert path.name.startswith("xoder-bash-")
