...
from xoder_coding.tui.terminal_title import (
...
def test_build_terminal_title_uses_session_name_and_running_frame() -> None:
    assert build_terminal_title("build notes", running=False) == "X | build notes"
    assert build_terminal_title("build notes", running=True, frame=1) == "⠙ X | build notes"


def test_build_terminal_title_falls_back_for_unnamed_sessions() -> None:
    assert build_terminal_title(None, running=False) == "X"
    assert build_terminal_title(" Untitled session ", running=True) == "⠋ X"
...
def test_terminal_title_supported_requires_tty_and_allows_opt_out() -> None:
    ...
    assert not terminal_title_supported(
        environ={"TERM": "xterm-256color", "XODER_TERMINAL_TITLE": "0"},
        stream=TtyStringIO(),
    )
    assert terminal_title_supported(
