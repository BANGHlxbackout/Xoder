...
from xoder_coding.tui.config import TuiSettings
...
def test_transcript_roles_are_shared_with_tui_state() -> None:
    from xoder_coding.tui.state import ChatItemRole
    from xoder_coding.tui.themes import TranscriptRole
...
def test_builtin_themes_are_loaded_from_packaged_json() -> None:
    assert BUILTIN_TUI_THEME_NAMES == ("xoder-dark", "xoder-light", "high-contrast")
    assert XODER_DARK_THEME.dark is True
    assert XODER_LIGHT_THEME.dark is False
    assert get_tui_theme("xoder-dark").screen_background == "#000000"
    assert get_tui_theme("high-contrast").prompt_border == "#00ff66"
    assert get_tui_theme("xoder-light").syntax_theme == "ansi_light"


def test_builtin_theme_resource_names_are_xoder_only() -> None:
    resources = files("xoder_coding.tui.themes")

    assert (resources / "xoder-dark.json").is_file()
    assert (resources / "xoder-light.json").is_file()
    assert not (resources / "tau-dark.json").is_file()
    assert not (resources / "tau-light.json").is_file()


def test_old_tau_theme_names_are_not_registered() -> None:
    assert "tau-dark" not in BUILTIN_TUI_THEME_NAMES
    assert "tau-light" not in BUILTIN_TUI_THEME_NAMES
    with pytest.raises(KeyError):
        get_tui_theme("tau-dark")
    with pytest.raises(KeyError):
        get_tui_theme("tau-light")
...
def test_resolved_theme_falls_back_to_xoder_dark_for_unknown_names() -> None:
    settings = TuiSettings(theme="missing-theme")

    assert settings.resolved_theme == XODER_DARK_THEME
...
def test_load_custom_themes_skips_builtin_shadowing_with_diagnostic(tmp_path: Path) -> None:
    themes_dir = tmp_path / "themes"
    _write_theme(themes_dir, "xoder-dark.json", _theme_data(name="xoder-dark"))

    themes, diagnostics = load_custom_tui_themes([themes_dir])

    assert themes == {}
    assert any("xoder-dark" in diagnostic.message for diagnostic in diagnostics)
...
def test_tool_result_accents_derive_from_theme() -> None:
    ...
    from xoder_coding.tui.state import ChatItem
    from xoder_coding.tui.widgets import render_chat_item
...
def test_resource_paths_expose_theme_dirs(tmp_path: Path) -> None:
    paths = XoderResourcePaths(root=tmp_path / ".xoder", cwd=tmp_path / "project")

    assert paths.themes_dirs == (
        tmp_path / ".xoder" / "themes",
        tmp_path / "project" / ".xoder" / "themes",
    )
