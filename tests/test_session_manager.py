...
def test_session_manager_creates_and_lists_sessions(tmp_path: Path) -> None:
    ...
    assert record.path.parent.parent == tmp_path / ".xoder" / "sessions"
    ...
    assert not (tmp_path / ".xoder" / "sessions" / "index.jsonl").exists()
...
def test_session_manager_ignores_legacy_tau_session_index(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    home = tmp_path / "home"
    legacy_index = home / ".tau" / "sessions" / "index.jsonl"
    legacy_index.parent.mkdir(parents=True)
    legacy_index.write_text(
        json.dumps(
            {
                "id": "legacy",
                "path": str(legacy_index.parent / "legacy.jsonl"),
                "cwd": str(tmp_path),
                "model": "legacy",
