...
from xoder_agent import AgentTool, AgentToolResult
from xoder_coding import Skill
...
def test_default_prompt_includes_tools_guidelines_date_and_cwd(tmp_path: Path) -> None:
    ...
    assert "You are an expert coding assistant operating inside Xoder" in prompt
    ...
    assert "Xoder documentation (read only when the user asks about Xoder itself" in prompt
...
def test_custom_prompt_replaces_default_but_keeps_append_context_and_date(tmp_path: Path) -> None:
    ...
    assert "Xoder documentation" not in prompt
