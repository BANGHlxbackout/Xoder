...
from xoder_coding import XoderResourcePaths
from xoder_coding.extensions import ExtensionRuntime
...
async def test_hello_tool_greets(tmp_path: Path) -> None:
    ...
    result = await hello.execute("test-call", {"who": "Xoder"})

    assert result.text == "Hello, Xoder!"
