...
def test_load_prompt_templates_missing_directory_returns_empty(tmp_path: Path) -> None:
    assert load_prompt_templates(XoderResourcePaths(root=tmp_path, agents_root=None)) == []
...
def test_load_prompt_templates_from_markdown_files(tmp_path: Path) -> None:
    ...
    templates = load_prompt_templates(XoderResourcePaths(root=tmp_path, agents_root=None))
...
def test_load_prompt_templates_with_diagnostics_reports_overrides(tmp_path: Path) -> None:
    ...
    agents_home = tmp_path / "home" / ".agents"
    cwd = tmp_path / "project"
    (xoder_home / "prompts").mkdir(parents=True)
    (xoder_home / "prompts" / "review.md").write_text("User Xoder review", encoding="utf-8")
    (cwd / ".xoder" / "prompts").mkdir(parents=True)
    (cwd / ".xoder" / "prompts" / "review.md").write_text(
        "Project Xoder review",
        encoding="utf-8",
    )
    ...
    assert templates[0].path == cwd / ".xoder" / "prompts" / "review.md"
