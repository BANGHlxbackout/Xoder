...
from xoder_coding.commands import create_default_command_registry
...
from xoder_coding.tui.widgets import render_completion_suggestions


def test_ignored_file_completion_dirs_uses_xoder_namespace() -> None:
    assert ".xoder" in IGNORED_FILE_COMPLETION_DIRS
    assert ".tau" not in IGNORED_FILE_COMPLETION_DIRS
...
def test_theme_argument_completion_uses_theme_names() -> None:
    state = build_completion_state(
        ...
        skills=(),
        prompt_templates=(),
        theme_names=("xoder-dark", "xoder-light", "high-contrast"),
    )

    assert [item.display for item in state.items] == ["xoder-dark", "xoder-light"]
    assert state.selected is not None
    assert state.selected.apply("/theme xoder-") == "/theme xoder-dark"
