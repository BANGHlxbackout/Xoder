...
from xoder_agent import AssistantMessage, ToolCall, UserMessage
from xoder_agent.messages import AgentMessage, assistant_content
from xoder_agent.session import CustomEntry, JsonlSessionStorage, LeafEntry, MessageEntry
from xoder_agent.tools import AgentTool, AgentToolResult
from xoder_agent.types import JSONValue
from xoder_ai import FakeProvider
from xoder_coding import CodingSession, CodingSessionConfig, XoderResourcePaths
from xoder_coding.extensions import (
    CustomMessageView,
    ExtensionAPI,
    ExtensionError,
    ExtensionRuntime,
...
def _paths(tmp_path: Path) -> XoderResourcePaths:
    return XoderResourcePaths(
        root=tmp_path / "home-xoder",
...
def _user_extensions_dir(paths: XoderResourcePaths) -> Path:
    return paths.root / "extensions"


def _project_extensions_dir(paths: XoderResourcePaths) -> Path:
    assert paths.cwd is not None
    return paths.cwd / ".xoder" / "extensions"
...
def test_discovers_user_extensions_and_skips_project_by_default(tmp_path: Path) -> None:
    ...
    _write_extension(_user_extensions_dir(paths), "user_ext", "def setup(xoder):\n    pass\n")
    _write_extension(_project_extensions_dir(paths), "proj_ext", "def setup(xoder):\n    pass\n")
...
def test_project_extensions_load_first_when_enabled(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_extension(_user_extensions_dir(paths), "ext_a", "def setup(xoder):\n    pass\n")
    _write_extension(_project_extensions_dir(paths), "ext_b", "def setup(xoder):\n    pass\n")
...
def test_legacy_project_extensions_directory_is_ignored(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    assert paths.cwd is not None
    _write_extension(paths.cwd / ".tau" / "extensions", "legacy", "def setup(xoder):\n    pass\n")
...
def test_duplicate_extension_names_prefer_first_loaded(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_extension(_user_extensions_dir(paths), "dup", "def setup(xoder):\n    pass\n")
    _write_extension(_project_extensions_dir(paths), "dup", "def setup(xoder):\n    pass\n")
...
def test_explicit_extension_paths_load_even_with_discovery_disabled(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_extension(_user_extensions_dir(paths), "skipped", "def setup(xoder):\n    pass\n")
    explicit = _write_extension(tmp_path / "elsewhere", "explicit", "def setup(xoder):\n    pass\n")
...
def test_manifest_wins_over_root_extension_py(tmp_path: Path) -> None:
    ...
    repo = tmp_path / "repo"
    _write_src_layout_extension(repo)
    (repo / "extension.py").write_text("def setup(xoder):\n    pass\n", encoding="utf-8")
    (repo / "pyproject.toml").write_text(
        '[tool.xoder]\nextensions = ["src/my_ext/extension.py"]\n', encoding="utf-8"
    )
...
def test_manifest_multiple_entries_load_independently(tmp_path: Path) -> None:
    ...
    other.mkdir()
    (other / "extension.py").write_text("def setup(xoder):\n    pass\n", encoding="utf-8")
    (repo / "pyproject.toml").write_text(
...
def test_manifest_duplicate_entry_names_first_wins(tmp_path: Path) -> None:
    ...
    for parent in ("src", "legacy"):
        ...
        package_dir.mkdir(parents=True)
        (package_dir / "extension.py").write_text("def setup(xoder):\n    pass\n", encoding="utf-8")
    (repo / "pyproject.toml").write_text(
...
def test_manifest_empty_list_falls_back_to_extension_py(tmp_path: Path) -> None:
    ...
    repo.mkdir()
    (repo / "extension.py").write_text("def setup(xoder):\n    pass\n", encoding="utf-8")
    (repo / "pyproject.toml").write_text("[tool.xoder]\nextensions = []\n", encoding="utf-8")
...
def test_manifest_without_xoder_table_falls_back_to_extension_py(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "extension.py").write_text("def setup(xoder):\n    pass\n", encoding="utf-8")
...
def test_manifest_invalid_declarations_are_error_diagnostics(tmp_path: Path) -> None:
    ...
    assert any(
        diag.severity == "error"
        and "tool.xoder.extensions" in diag.message
        and "list of file paths" in diag.message
        for diag in diagnostics
    )


def test_legacy_manifest_namespace_is_ignored(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    repo = tmp_path / "repo"
    _write_src_layout_extension(repo)
    (repo / "pyproject.toml").write_text(
    ...
    assert discovered == ()
    assert any("no extensions found" in diagnostic.message for diagnostic in diagnostics)
...
def test_helper_modules_stay_namespaced_in_sys_modules(tmp_path: Path) -> None:
    ...
    assert any(
        name.startswith("xoder_extension_pkg_ns") and name.endswith(".helper")
...
def test_broken_extension_is_isolated(tmp_path: Path) -> None:
    ...
    _write_extension(_user_extensions_dir(paths), "works", "def setup(xoder):\n    pass\n")
...
def test_async_setup_is_rejected(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_extension(
        _user_extensions_dir(paths), "async_setup", "async def setup(xoder):\n    pass\n"
    )
...
def test_underscore_files_are_skipped(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _write_extension(_user_extensions_dir(paths), "_private", "def setup(xoder):\n    pass\n")
...
def test_setup_failure_rolls_back_registrations(tmp_path: Path) -> None:
    ...
    _write_extension(
        ...
        (
            ...
            "def setup(xoder):\n"
            "    xoder.register_tool(AgentTool(name='t', description='d',"
...
async def test_extension_tool_overrides_builtin_by_name(tmp_path: Path) -> None:
    ...
    _write_extension(
        ...
        (
            ...
            "def setup(xoder):\n"
            "    xoder.register_tool(\n"
            "        AgentTool(name='read', label='Read', description='replacement',\n"
            "                  parameters={}, execute_fn=_run)\n"
            "    )\n"
        ),
    )
...
def test_extension_commands_layer_onto_default_registry(tmp_path: Path) -> None:
    ...
    _write_extension(
        ...
        (
            ...
            "    return f'echo: {args}'\n\n\n"
            "def setup(xoder):\n"
            "    xoder.register_command('echo', _handler, description='Echo args.')\n"
        ),
    )
...
def test_extension_command_cannot_shadow_builtin(tmp_path: Path) -> None:
    ...
    _write_extension(
        ...
        (
            "def setup(xoder):\n"
            "    xoder.register_command('model', lambda args, context: 'hijacked')\n"
        ),
    )
...
def test_extension_command_errors_are_contained(tmp_path: Path) -> None:
    ...
    _write_extension(
        ...
        (
            ...
            "def setup(xoder):\n"
            "    xoder.register_command('boom', _handler)\n"
        ),
    )
...
def _command_context(registry: object, text: str, name: str, args: str) -> object:
    from xoder_coding.commands import CommandContext
...
def test_unknown_event_subscription_is_a_diagnostic(tmp_path: Path) -> None:
    ...
    _write_extension(
        _user_extensions_dir(paths),
        "bad_event",
        "def setup(xoder):\n    xoder.on('no_such_event', lambda event, context: None)\n",
    )
...
async def test_tool_call_hook_can_rewrite_arguments(tmp_path: Path) -> None:
    ...
    api.on("tool_call", lambda event, context: ToolCallHookResult(arguments={"who": "xoder"}))
...
async def test_agent_event_fan_out_and_wildcard(tmp_path: Path) -> None:
    ...
    from xoder_agent.events import ToolExecutionStartEvent, TurnStartEvent
...
async def test_extension_turn_events_include_pi_session_metadata(tmp_path: Path) -> None:
    from xoder_agent.events import AgentStartEvent
    from xoder_agent.events import TurnEndEvent as AgentTurnEndEvent
    from xoder_agent.events import TurnStartEvent as AgentTurnStartEvent
    from xoder_agent.messages import UserMessage
    from xoder_coding.extensions import TurnEndEvent, TurnStartEvent
...
async def test_extension_turn_index_resets_for_each_agent_run(tmp_path: Path) -> None:
    ...
    from xoder_agent.events import TurnStartEvent as AgentTurnStartEvent
    from xoder_agent.messages import UserMessage
    from xoder_coding.extensions import TurnStartEvent
...
async def test_message_end_event_surfaces_provider_usage(tmp_path: Path) -> None:
    ...
    from xoder_agent import Usage
    from xoder_agent.events import MessageEndEvent
    from xoder_coding.extensions.api import ExtensionAPI
...
async def test_raising_event_handler_is_recorded(tmp_path: Path) -> None:
    ...
    from xoder_agent.events import TurnStartEvent
...
class RecordingUiBridge:
    ...
    def theme(self):  # noqa: ANN202 - TuiTheme, imported lazily by the default
        from xoder_coding.tui.config import XODER_DARK_THEME
    ...
    def open_main_view(self, factory):  # noqa: ANN001
        ...
        from xoder_coding.extensions.api import _DeadMainViewHandle
...
async def test_headless_ui_bridges_return_pi_defaults(tmp_path: Path) -> None:
    from xoder_coding.extensions import NullUiBridge, StderrUiBridge
...
async def test_sync_command_spawns_task_that_awaits_dialog(tmp_path: Path) -> None:
    ...
    _write_extension(
        ...
        (
            ...
            "    return 'opening menu...'\n\n\n"
            "def setup(xoder):\n"
            "    xoder.register_command('menu', _handler)\n"
        ),
    )
...
def _register_inline_extension(runtime: ExtensionRuntime, name: str) -> object:
    from xoder_coding.extensions.loader import LoadedExtension
...
async def test_session_exposes_extension_tools_and_commands(tmp_path: Path) -> None:
    body = HELLO_TOOL_EXTENSION + (
        ...
        "def setup(xoder):\n"
        "    _original_setup(xoder)\n"
        "    xoder.register_command('hi', _cmd, description='Say hi.')\n"
    )
...
async def test_extension_guideline_reaches_system_prompt(tmp_path: Path) -> None:
    body = "def setup(xoder):\n    xoder.add_prompt_guideline('Never commit directly to main')\n"
    session = await CodingSession.load(
        _session_config(tmp_path, FakeProvider([]), extension_body=body)
    )
...
async def test_reload_picks_up_guideline_changes(tmp_path: Path) -> None:
    ...
    _write_extension(
        _user_extensions_dir(paths),
        "late_guideline",
        "def setup(xoder):\n    xoder.add_prompt_guideline('Prefer uv over pip')\n",
    )
...
async def test_session_start_deferred_until_host_emits(tmp_path: Path) -> None:
    body = (
        "EVENTS = []\n\n\n"
        "def setup(xoder):\n"
        "    xoder.on('session_start', lambda event, context: EVENTS.append(event.reason))\n"
    )
    session = await CodingSession.load(
        _session_config(tmp_path, FakeProvider([]), extension_body=body)
    )
...
async def test_session_start_handler_can_notify_through_attached_bridge(
    ...
    body = (
        "def setup(xoder):\n"
        "    xoder.on('session_start', "
        "lambda event, context: xoder.notify('loaded', 'info'))\n"
    )
    session = await CodingSession.load(
        _session_config(tmp_path, FakeProvider([]), extension_body=body)
    )
...
async def test_agent_events_reach_extension_after_interrupted_tool_repair(
    ...
    body = (
        "EVENTS = []\n\n\n"
        "def setup(xoder):\n"
        "    xoder.on('agent_event', lambda event, context: EVENTS.append(event.type))\n"
    )
    provider = FakeProvider(
        [
            [
...
async def test_extension_tool_call_block_reaches_model(tmp_path: Path) -> None:
    body = (
        "from xoder_coding.extensions import ToolCallHookResult\n\n\n"
        "def _hook(event, context):\n"
        "    if event.tool_name == 'bash':\n"
        "        return ToolCallHookResult(block=True, reason='no shell today')\n"
        "    return None\n\n\n"
        "def setup(xoder):\n"
        "    xoder.on('tool_call', _hook)\n"
    )
    provider = FakeProvider(
        [
            [
...
async def test_runtime_survives_new_session_swap(tmp_path: Path) -> None:
    from xoder_coding import SessionManager
    ...
    body = (
        ...
        "    xoder.on('session_shutdown', "
        "lambda event, context: EVENTS.append(('stop', event.reason)))\n"
    )
    from dataclasses import replace as dataclass_replace

    from xoder_coding import XoderPaths

    provider = FakeProvider([])
    manager = SessionManager(
        XoderPaths(home=tmp_path / "home-xoder", agents_home=tmp_path / "home-agents")
    )
...
async def test_session_swap_clears_host_extension_components(tmp_path: Path) -> None:
    ...
    from xoder_coding import SessionManager, XoderPaths
...
async def test_reload_awaits_shutdown_before_invalidation_and_starts_new_generation(
    ...
    body = (
        ...
        "        EVENTS.append(('stop', event.reason, xoder.name))\n"
        "    @xoder.on('session_start')\n"
        "    async def start(event, context):\n"
        "        EVENTS.append(('start', event.reason, xoder.name))\n"
        "        xoder.context.ui.components.set_slot_widget('status', [event.reason])\n"
    )
    session = await CodingSession.load(
        _session_config(tmp_path, FakeProvider([]), extension_body=body)
    )
...
def _loaded_extension_module(name: str) -> object:
    candidates = [
        ...
        if module_name.startswith(f"xoder_extension_{name}") and "." not in module_name
    ]
