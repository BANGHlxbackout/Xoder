...
from xoder_ai import CancellationToken, FakeProvider, ModelProvider, RuntimeModelLimits
from xoder_ai.events import AssistantMessageEvent
from xoder_coding import (
    CodingSession,
    CodingSessionConfig,
    FileCredentialStore,
    ModelChoice,
...
from xoder_coding import session as coding_session_module
from xoder_coding.events import QueueUpdateEvent
from xoder_coding.session import _ordered_tree_entries, parse_terminal_command
...
@pytest.mark.anyio
async def test_session_export_defaults_to_xoder_filename_in_cwd(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / ".xoder" / "sessions" / "session-1.jsonl")
    session = await CodingSession.load(_config(tmp_path, FakeProvider([]), storage))
    await storage.append(MessageEntry(id="root", message=UserMessage(content="Export me")))

    output_path = await session.export()

    assert output_path == tmp_path / "xoder-session.html"
    html = output_path.read_text(encoding="utf-8")
    assert "<title>Xoder Session Export</title>" in html
    assert "Export me" in html
    assert str(storage.path) in html


def test_session_export_without_storage_path_uses_xoder_filename(tmp_path: Path) -> None:
    assert (
        coding_session_module._resolve_export_destination(
            None,
            cwd=tmp_path,
            session_path=None,
            format="html",
        )
        == tmp_path / "xoder-session.html"
    )
    assert (
        coding_session_module._resolve_export_destination(
            None,
...
async def test_session_export_writes_jsonl_to_destination_directory(tmp_path: Path) -> None:
    ...
    assert output_path == tmp_path / "exports" / "xoder-session.jsonl"
    assert "Export me" in output_path.read_text(encoding="utf-8")


@pytest.mark.anyio
async def test_session_export_preserves_explicit_destination_filename(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / ".xoder" / "sessions" / "session-1.jsonl")
    session = await CodingSession.load(_config(tmp_path, FakeProvider([]), storage))
    await storage.append(MessageEntry(id="root", message=UserMessage(content="Export me")))

    output_path = await session.export(Path("exports") / "named-export.html")

    assert output_path == tmp_path / "exports" / "named-export.html"
    assert "Export me" in output_path.read_text(encoding="utf-8")
...
async def test_prompt_logs_unexpected_agent_call_exception(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    xoder_paths = XoderPaths(home=tmp_path / "xoder-home", agents_home=tmp_path / "agents-home")
    session = await CodingSession.load(
        CodingSessionConfig(
            ...
            resource_paths=XoderResourcePaths(root=xoder_paths.home, paths=xoder_paths),
        )
    )
    ...
    log_path = xoder_paths.agent_calls_log_path
...
async def test_tree_can_branch_from_first_user_message_before_assistant_response(
    ...
    with pytest.raises(RuntimeError, match="Xoder is still working"):
...
async def test_session_persists_thinking_preference_for_new_sessions(tmp_path: Path) -> None:
    xoder_paths = XoderPaths(home=tmp_path / ".xoder")
    ...
    saved = load_provider_settings(xoder_paths)
...
async def test_session_builds_system_prompt_when_system_is_omitted(tmp_path: Path) -> None:
    ...
    config = CodingSessionConfig(
        ...
        storage=storage,
        cwd=tmp_path,
        resource_paths=XoderResourcePaths(root=resource_root, agents_root=None),
    )
    session = await CodingSession.load(config)
...
async def test_session_touches_session_manager_after_persisting_messages(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    manager = SessionManager(XoderPaths(home=tmp_path / ".xoder", agents_home=tmp_path / ".agents"))
    ...
    config = CodingSessionConfig(
        ...
        resource_paths=XoderResourcePaths(root=tmp_path / "resources", agents_root=None),
    )
    session = await CodingSession.load(config)
...
async def test_system_command_shows_prompt_without_persisting_or_adding_context(
    ...
    assert result.message == "You are Xoder."
...
async def test_session_reload_skips_provider_settings_refresh(
    ...
    def fail_load_provider_settings(paths: XoderPaths | None = None) -> ProviderSettings:
        del paths
...
async def test_session_provider_settings_reload_uses_session_paths(
    ...
    xoder_paths = XoderPaths(home=tmp_path / "xoder-home", agents_home=tmp_path / "agents-home")
    seen_paths: list[XoderPaths | None] = []

    def load_provider_settings(paths: XoderPaths | None = None) -> ProviderSettings:
...
async def test_session_switch_uses_session_credential_store(
    ...
    xoder_paths = XoderPaths(home=tmp_path / "xoder-home", agents_home=tmp_path / "agents-home")
    FileCredentialStore(xoder_paths.home / "credentials.json").set("openai", "stored-key")
    ...
    assert credential_store_paths == [xoder_paths.home / "credentials.json"]
...
async def test_session_toggles_and_cycles_scoped_models(
    ...
    saved = json.loads((xoder_paths.home / "providers.json").read_text(encoding="utf-8"))
...
async def test_session_toggle_scoped_model_preserves_newer_provider_file_changes(
    ...
    save_provider_settings(newer_settings, xoder_paths)
    session = await CodingSession.load(
        CodingSessionConfig(
            provider=FakeProvider([]),
            model="qwen",
    ...
    saved = coding_session_module.load_provider_settings(xoder_paths)
...
async def test_session_set_model_persists_default_provider_model(
    ...
    isolate_home(monkeypatch, tmp_path)
    xoder_paths = XoderPaths(home=tmp_path / ".xoder", agents_home=tmp_path / ".agents")
    provider_config = OpenAICompatibleProviderConfig(
        name="openai",
...
async def test_session_set_model_preserves_newer_provider_file_changes(
    ...
    saved = coding_session_module.load_provider_settings(xoder_paths)
    assert saved.get_provider("openai").default_model == "gpt-5-mini"
    assert saved.get_provider("openrouter").headers == {"X-Title": "Xoder"}
    assert saved.scoped_models == (
