...
async def test_subscribers_receive_nested_message_updates_and_unsubscribe() -> None:
    ...
    harness = AgentHarness(
        AgentHarnessConfig(provider=provider, model="fake", system="You are Xoder.")
    )
    seen: list[str] = []
...
async def test_harness_passes_canonical_tools_to_loop() -> None:
    ...
    harness = AgentHarness(
        AgentHarnessConfig(provider=provider, model="fake", system="You are Xoder.", tools=[tool])
    )
...
def test_queue_mutators_return_canonical_snapshots() -> None:
    harness = AgentHarness(
        AgentHarnessConfig(provider=FakeProvider([]), model="fake", system="You are Xoder.")
    )
