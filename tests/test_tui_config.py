...
def test_tui_settings_path_uses_xoder_home(tmp_path: Path) -> None:
    paths = XoderPaths(home=tmp_path / ".xoder", agents_home=tmp_path / ".agents")

    assert tui_settings_path(paths) == tmp_path / ".xoder" / "tui.json"
...
def test_load_tui_settings_returns_defaults_when_file_is_missing(tmp_path: Path) -> None:
    ...
    assert load_tui_settings(paths).theme == "xoder-dark"
...
def test_save_tui_settings_writes_json(tmp_path: Path) -> None:
    ...
    path = save_tui_settings(TuiSettings(theme="xoder-light"), paths)

    assert path == tmp_path / ".xoder" / "tui.json"
    assert load_tui_settings(paths).theme == "xoder-light"
...
def test_tui_settings_accept_unknown_theme_and_fall_back_when_resolving() -> None:
    ...
    assert settings.resolved_theme == get_tui_theme("xoder-dark")
...
def test_tui_settings_accept_light_theme() -> None:
    settings = tui_settings_from_json({"theme": "xoder-light"})

    assert settings.theme == "xoder-light"
