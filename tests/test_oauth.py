...
from xoder_coding.oauth import (
...
def test_create_openai_codex_authorization_flow_includes_pkce_and_codex_params() -> None:
    flow = create_openai_codex_authorization_flow(originator="xoder-test")
    ...
    assert params["originator"] == ["xoder-test"]
    assert params["state"] == [flow.state]
    assert params["code_challenge"][0]
    assert flow.verifier


def test_create_openai_codex_authorization_flow_uses_xoder_originator_by_default() -> None:
    flow = create_openai_codex_authorization_flow()

    assert parse_qs(urlparse(flow.url).query)["originator"] == ["xoder"]


@pytest.mark.anyio
async def test_local_oauth_server_uses_xoder_callback_host_and_ignores_legacy_name(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("XODER_OAUTH_CALLBACK_HOST", "127.0.0.1")
    monkeypatch.setenv("TAU_OAUTH_CALLBACK_HOST", "not-a-host")

    server = await _start_local_oauth_server("state", callback_port=0)
