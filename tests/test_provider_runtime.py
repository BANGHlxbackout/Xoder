...
from xoder_ai import AnthropicProvider, OpenAICodexProvider, OpenAICompatibleProvider
from xoder_coding import provider_runtime
...
from xoder_coding.provider_runtime import OpenAICodexCredentialResolver, create_model_provider
...
def test_create_model_provider_uses_anthropic_oauth_runtime_auth(tmp_path) -> None:
    ...
    assert provider._config.headers["user-agent"] == "claude-cli/xoder"
...
def test_create_model_provider_coerces_unsupported_startup_thinking_level(
    ...
    monkeypatch.setenv("XODER_TEST_KIMI_CODE_API_KEY", "test-key")
    store = FileCredentialStore(tmp_path / "credentials.json")
    provider_config = OpenAICompatibleProviderConfig(
        name="kimi-code",
        api_key_env="XODER_TEST_KIMI_CODE_API_KEY",
        models=("k3",),
        default_model="k3",
