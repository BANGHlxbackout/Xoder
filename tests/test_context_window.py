...
from xoder_agent import AssistantMessage, TextContent, ToolCall, ToolResultMessage, UserMessage
...
def test_context_token_estimate_includes_system_messages_and_tools(tmp_path: Path) -> None:
    ...
    assert estimate > estimate_text_tokens("You are Xoder.hellohi")
...
def test_context_usage_estimate_reports_breakdown(tmp_path: Path) -> None:
    ...
    usage = estimate_context_usage(system="You are Xoder.", messages=messages, tools=tools)

    assert isinstance(usage, ContextUsageEstimate)
    assert usage.message_count == 2
    assert usage.tool_count == len(tools)
    assert usage.system_tokens == estimate_text_tokens("You are Xoder.")
    ...
    assert estimate_context_tokens(system="You are Xoder.", messages=messages, tools=tools) == (
        usage.total_tokens
    )
