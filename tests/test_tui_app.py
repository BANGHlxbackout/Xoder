...
from xoder_agent.messages import assistant_content
from xoder_agent.provider_events import TextDeltaEvent, ThinkingDeltaEvent
from xoder_coding.catalog_loader import user_catalog_path
from xoder_coding.commands import CommandResult
from xoder_coding.credentials import FileCredentialStore, OAuthCredential
from xoder_coding.events import QueueUpdateEvent
from xoder_coding.paths import XoderPaths
from xoder_coding.prompt_templates import PromptTemplate
from xoder_coding.provider_config import (
...
from xoder_coding.session import (
    ModelChoice,
    SessionTreeBranchResult,
    SessionTreeChoice,
    TerminalCommandResult,
)
from xoder_coding.session_manager import CodingSessionRecord
from xoder_coding.session_stats import SessionStats
from xoder_coding.skills import Skill, format_skill_invocation
from xoder_coding.system_prompt import ProjectContextFile
from xoder_coding.tools import create_coding_tools
from xoder_coding.tui import app as tui_app
from xoder_coding.tui.app import (
...
from xoder_coding.tui.autocomplete import CompletionItem, CompletionState
from xoder_coding.tui.config import (
    HIGH_CONTRAST_THEME,
    XODER_DARK_THEME,
    XODER_LIGHT_THEME,
    TuiKeybindings,
...
from xoder_coding.tui.state import ChatItem, TuiState
from xoder_coding.tui.terminal_title import TerminalTitleController
from xoder_coding.tui.widgets import (
...
class FakeSession:
    def __init__(self, messages=(), events=()) -> None:
        ...
        self.system_prompt = "You are Xoder."
...
def _visible_footer_bindings(app: XoderTuiApp) -> dict[str, str]:
...
def test_session_sidebar_uses_prominent_title_and_accented_section_headers() -> None:
    ...
    assert str(session_name.renderable.style) == f"bold {XODER_DARK_THEME.accent}"
    assert activity_header.left == 1
    assert str(activity_header.renderable.style) == f"bold {XODER_DARK_THEME.prompt_text}"
    assert str(activity_content.renderable.style) == XODER_DARK_THEME.completion_description
...
def test_compact_session_info_styles_parent_path_as_metadata() -> None:
    cwd = _styled_cwd(Path("/workspace/project"), theme=XODER_DARK_THEME)

    assert cwd.plain == "/workspace/project (--)"
    assert str(cwd.spans[0].style) == XODER_DARK_THEME.completion_description
    assert str(cwd.spans[1].style) == XODER_DARK_THEME.prompt_text
    assert str(cwd.spans[2].style) == XODER_DARK_THEME.completion_description
...
def test_tui_state_compacts_expanded_skill_messages() -> None:
    skill = Skill(
        name="review",
        path=Path("/workspace/.xoder/skills/review.md"),
        content="# Review\nFull noisy instructions.",
                ...
                content=assistant_content(
                    ...
                    [
                        ToolCall(
                            id="call-1",
                            name="read",
                            arguments={"path": "/workspace/.xoder/skills/review.md"},
                        )
                    ],
                )
            ),
...
def test_assistant_markdown_titles_use_highlight_color_and_left_alignment() -> None:
    ...
    assert _style_color_escape(XODER_DARK_THEME.markdown_heading) in output
...
def test_dark_theme_markdown_bullets_use_theme_bullet_color() -> None:
    ...
    assert _style_color_escape(XODER_DARK_THEME.markdown_bullet) in output
...
def test_markdown_tables_use_highlight_color_for_headers() -> None:
    ...
    assert _style_color_escape(XODER_DARK_THEME.accent) in output
...
async def test_textual_markdown_widget_uses_theme_link_style() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        block = app.query_one(XoderMarkdownBlock)
    ...
    assert markdown.xoder_link_style == XODER_DARK_THEME.markdown_link
...
def test_textual_markdown_uses_theme_highlight_and_aqua_inline_code() -> None:
    variables = _theme_css_variables(XODER_LIGHT_THEME)

    assert variables["xoder-markdown-highlight"] == XODER_LIGHT_THEME.markdown_heading
    assert variables["xoder-markdown-table-header"] == XODER_LIGHT_THEME.markdown_table_header
    assert variables["xoder-markdown-table-border"] == XODER_LIGHT_THEME.markdown_table_border
    assert variables["xoder-markdown-inline-code"] == XODER_LIGHT_THEME.markdown_inline_code
    assert (
        variables["xoder-markdown-code-block-background"]
        == XODER_LIGHT_THEME.markdown_code_block_background
    )
    assert variables["xoder-markdown-link"] == XODER_LIGHT_THEME.markdown_link
...
def test_light_theme_markdown_code_uses_theme_highlight_without_background() -> None:
    ...
    console.print(
        render_chat_item(
            ChatItem(role="assistant", text="Use `xoder` here."),
            theme=XODER_LIGHT_THEME,
        )
    )
...
def test_pending_tool_invocation_uses_tool_accent_color() -> None:
    ...
    console.print(
        _transcript_plain_body_text(
            item,
            text=item.text,
            body_style=XODER_DARK_THEME.role_styles["tool"].body,
            theme=XODER_DARK_THEME,
        )
    )
...
async def test_streaming_transcript_applies_role_foreground() -> None:
    ...
    async with app.run_test(size=(60, 20)) as pilot:
        ...
        thinking_fg, _ = _split_rich_style_colors(XODER_DARK_THEME.role_styles["thinking"].body)
        assistant_fg, _ = _split_rich_style_colors(XODER_DARK_THEME.role_styles["assistant"].body)
        ...
        await transcript.append_thinking_delta(
            "reasoning", theme=XODER_DARK_THEME, show_thinking=True
        )
        await pilot.pause()
        thinking = next(
        ...
        await transcript.append_assistant_delta("answer", theme=XODER_DARK_THEME)
        await pilot.pause()
        assistant = next(
...
async def test_tool_completion_updates_row_without_redrawing_history() -> None:
    app = XoderTuiApp(FakeSession(messages=[UserMessage(content="earlier")]))
...
async def test_tool_transcript_uses_native_markdown_without_custom_selection_painting() -> None:
    app = XoderTuiApp(FakeSession(messages=[]))
    item = ChatItem(
        role="tool",
        text="→ read README.md",
        tool_result_text="✓ read\nREADME contents",
...
def test_xoder_markdown_block_is_not_selectable_until_mounted() -> None:
    markdown = TextualMarkdown("example")
    block = XoderMarkdownBlock(
        markdown,
...
@pytest.mark.anyio
async def test_xoder_markdown_block_remains_selectable_after_mount() -> None:
    app = XoderTuiApp(
        FakeSession([AssistantMessage(content="Read [docs](https://example.com).")]),
    )
...
async def test_tui_sidebar_shows_on_right_when_configured() -> None:
    app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="right"))
...
async def test_tui_sidebar_is_hidden_when_off() -> None:
    app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="off"))
...
async def test_tui_transcript_code_block_scrollbar_matches_overflow(
    ...
    app = XoderTuiApp(FakeSession(messages=[AssistantMessage(content=f"```python\n{code}\n```")]))
...
@pytest.mark.parametrize(
    "theme",
    [XODER_DARK_THEME, XODER_LIGHT_THEME, HIGH_CONTRAST_THEME],
    ids=lambda theme: theme.name,
)
async def test_transcript_code_fence_uses_theme_background(theme: TuiTheme) -> None:
    """Code fences keep Xoder's themed background for every built-in theme.

    Regression test: Textual's built-in `MarkdownFence:light` rule outranks
    Xoder's plain `ThemedMarkdownWidget MarkdownFence` selector, which used to
...
def test_tui_app_uses_configured_theme_css_variables() -> None:
    app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(theme="high-contrast"))

    variables = app.get_theme_variable_defaults()

    assert variables["xoder-screen-background"] == "#000000"
    assert variables["xoder-prompt-background"] == "#1a1a1a"
    assert variables["xoder-prompt-border"] == "#00ff66"
...
def test_tui_app_registers_and_applies_custom_theme() -> None:
    ...
    try:
        app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(theme="midnight"))

        assert app.theme == "midnight"
        assert app.get_theme_variable_defaults()["xoder-screen-background"] == "#123456"
    finally:
        set_custom_tui_themes({})


def test_tui_app_falls_back_to_xoder_dark_when_theme_is_missing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    isolate_home(monkeypatch, tmp_path)

    app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(theme="missing-theme"))

    assert app.theme == "xoder-dark"
...
def test_tui_app_uses_light_theme_css_variables() -> None:
    app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(theme="xoder-light"))
...
def test_tui_app_registers_only_xoder_themes_with_textual() -> None:
    app = XoderTuiApp(FakeSession())

    assert tuple(app.available_themes) == ("xoder-dark", "xoder-light", "high-contrast")


def test_textual_theme_mapping_uses_xoder_theme_values() -> None:
    textual_theme = _textual_theme_for_xoder_theme("xoder-light")

    assert textual_theme.name == "xoder-light"
    assert textual_theme.primary == XODER_LIGHT_THEME.accent
...
def test_autocomplete_and_picker_share_theme_highlight(theme: TuiTheme) -> None:
    ...
    assert variables["xoder-highlight-background"] == theme.highlight_background
    assert variables["xoder-highlight-text"] == theme.highlight_text


def test_xoder_light_theme_uses_light_chat_backgrounds() -> None:
    theme = TuiSettings(theme="xoder-light").resolved_theme
...
async def test_tui_app_updates_terminal_title_for_running_and_named_session() -> None:
    ...
    async with app.run_test():
        assert writes[-1] == "\x1b]0;X | build notes\x07"
        ...
        app._refresh_chrome()
        assert writes[-1] == "\x1b]0;⠙ X | ship notes\x07"

        app.adapter.apply(AgentEndEvent())
        app._refresh()
        assert writes[-1] == "\x1b]0;X | ship notes\x07"

    assert writes[-1] == "\x1b]0;X\x07"
...
async def test_tui_app_updates_terminal_title_after_auto_session_naming() -> None:
    ...
    app = XoderTuiApp(AutoNamingSession())
    writes: list[str] = []
    app._terminal_title = TerminalTitleController(enabled=True, writer=writes.append)
    ...
    async with app.run_test():
        ...
        assert "\x1b]0;X | Debug login\x07" in writes
        ...
        assert writes[-1] == "\x1b]0;X | Debug login\x07"
...
@pytest.mark.anyio
async def test_textual_theme_change_persists_xoder_theme(
    ...
    async with app.run_test() as pilot:
        app.theme = "xoder-light"
        await pilot.pause()

    assert app.tui_settings.theme == "xoder-light"
    assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "xoder-light"') != -1
...
async def test_tui_app_theme_command_opens_picker_and_persists_selection(
    ...
    async with app.run_test() as pilot:
        ...
        assert app.tui_settings.theme == "xoder-light"
        assert app.theme == "xoder-light"
        assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "xoder-light"') != -1
        assert app.get_theme_variable_defaults()["xoder-screen-background"] == "#ffffff"
...
async def test_theme_picker_highlight_uses_theme_selection_palette(theme: TuiTheme) -> None:
    theme_names = (XODER_DARK_THEME.name, XODER_LIGHT_THEME.name, HIGH_CONTRAST_THEME.name)
    app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(theme=theme.name))
...
async def test_extension_select_dialog_returns_choice() -> None:
    app = XoderTuiApp(FakeSession())  # type: ignore[arg-type]
...
async def test_restored_tool_calls_render_via_render_call() -> None:
    ...
    app = XoderTuiApp(session)  # type: ignore[arg-type]
...
async def test_tui_app_theme_command_argument_updates_theme_and_persists(
    ...
    async with app.run_test() as pilot:
        prompt = app.query_one("#prompt")
        prompt.value = "/theme xoder-light"
        await pilot.press("enter")
...
async def test_tui_app_blocks_tree_picker_while_agent_is_running() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert notifications == [
            "Xoder is still working. Press Escape to interrupt before using /tree."
        ]
...
async def test_tui_app_tree_picker_branches_with_summary() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert str(rendered_labels[0].spans[0].style) == _style_rgb(XODER_DARK_THEME.accent)
        assert str(rendered_labels[3].spans[0].style) == _style_rgb(XODER_DARK_THEME.highlight_text)
        ...
        assert str(left_label.spans[0].style) == _style_rgb(XODER_DARK_THEME.highlight_text)
        assert str(right_label.spans[0].style) == _style_rgb(XODER_DARK_THEME.accent)
        await pilot.press("s")
        await pilot.pause()
...
async def test_tui_app_session_modal_auto_copies_selected_text(
    ...
    app = XoderTuiApp(FakeSession(), tui_settings=TuiSettings(auto_copy_selection=False))
    copied: list[str] = []
    monkeypatch.setattr(app, "copy_to_clipboard", copied.append)
...
async def test_tui_login_saves_provider_key(
    ...
    assert (tmp_path / ".xoder" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_openai_codex_saves_oauth_credentials(
    ...
    credentials = (tmp_path / ".xoder" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_custom_provider_writes_catalog_and_preferences(
    ...
    paths = XoderPaths(home=tmp_path / ".xoder")
    ...
    assert FileCredentialStore(tmp_path / ".xoder" / "credentials.json").get("nebius") == (
        "stored-nebius-key"
    )
    assert session.provider_reload_count == 1
...
async def test_tui_login_preserves_existing_scoped_models_and_providers(
    ...
    assert FileCredentialStore(xoder_home / "credentials.json").get("openrouter") == (
...
async def test_tui_logout_removes_stored_api_key(
    ...
    isolate_home(monkeypatch, tmp_path)
    credential_path = tmp_path / ".xoder" / "credentials.json"
...
async def test_tui_logout_opens_stored_credential_provider_picker(
    ...
    isolate_home(monkeypatch, tmp_path)
    FileCredentialStore(tmp_path / ".xoder" / "credentials.json").set("anthropic", "stored-key")
    app = XoderTuiApp(FakeSession())
...
async def test_tui_app_up_arrow_preserves_non_empty_prompt_movement() -> None:
    app = XoderTuiApp(FakeSession(messages=[UserMessage(content="remembered prompt")]))
...
async def test_tui_prompt_ctrl_c_clears_text() -> None:
    app = XoderTuiApp(FakeSession(messages=(UserMessage(content="User prompt"),)))
...
async def test_tui_prompt_worker_refreshes_directly() -> None:
    app = XoderTuiApp(FakeSession(events=[AgentStartEvent(), AgentEndEvent()]))
    refreshes = 0
...
async def test_tui_prompt_worker_shows_diagnostic_log_path_for_error_event(tmp_path: Path) -> None:
    class ErrorSession(FakeSession):
        def __init__(self) -> None:
            ...
            self.last_diagnostic_log_path = tmp_path / "xoder-home" / "logs" / "agent-calls.jsonl"
...
async def test_tui_app_shows_startup_update_notice_in_transcript_only() -> None:
    ...
    app = XoderTuiApp(
        session,
        startup_notices=("Xoder updated to 0.2.0", "Xoder 0.2.0 is available"),
    )
    notifications: list[tuple[str, str | None]] = []
    ...
    async with app.run_test() as pilot:
        ...
        transcript = app.query_one("#transcript", TranscriptView)
        assert [line.text for line in transcript.lines] == [
            "Xoder updated to 0.2.0",
            "Xoder 0.2.0 is available",
            "Earlier prompt",
        ]
...
async def test_tui_app_runs_initial_prompt() -> None:
    ...
    app = XoderTuiApp(session, initial_prompt="explain this repo")
...
async def test_run_tui_app_falls_back_to_first_credentialed_provider(
    ...
    monkeypatch.setattr(tui_app, "XoderTuiApp", FakeApp)
...
async def test_run_tui_app_opens_when_provider_login_is_missing(
    ...
    class FakeApp:
        def __init__(self, session: str, **kwargs: object) -> None:
            ...
            assert "Xoder 0.2.0 is available" not in message
            ...
            assert "Xoder 0.2.0 is available" in notices
    ...
    await tui_app.run_tui_app(
        ...
        model=None,
        session_manager=FakeManager(),
        startup_notice="Xoder 0.2.0 is available",
    )
...
# These drive the generic widget-hosting seam on the real XoderTuiApp via the
...
def _component_bridge(app: XoderTuiApp) -> _TuiExtensionUiBridge:
...
async def test_component_key_interceptor_preempts_priority_binding() -> None:
    ...
    XoderTuiApp.on_event, ahead of that priority check, so it can own those keys.
    """
    app = XoderTuiApp(FakeSession())
