...
async def test_openai_compatible_provider_formats_request_and_streams_text() -> None:
    ...
    assert payload["messages"] == [
        {"role": "system", "content": "You are Xoder."},
...
async def test_google_provider_sends_system_instruction_at_top_level() -> None:
    ...
    assert payload["systemInstruction"] == {"parts": [{"text": "You are Xoder."}]}
...
@pytest.mark.anyio
async def test_openai_compatible_provider_preserves_tool_metadata_across_stream_deltas() -> None:
    tools = [
        _provider_tool("tool_alpha", "First generic tool.", {"type": "object"}),
        _provider_tool("tool_beta", "Second generic tool.", {"type": "object"}),
    ]
    ...
    def handler(_request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            ...
            text=(
                ...
                '{"index":0,"id":"call-alpha","function":{"name":"tool_alpha",'
                '"arguments":"{\\"value\\":"}},'
                '{"index":1,"id":"call-beta","function":{"name":"tool_beta",'
                '"arguments":"{\\"value\\":"}}]}}]}\n\n'
                'data: {"choices":[{"delta":{"tool_calls":['
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        ...
        events = await _collect(
            provider.stream_response(
                ...
                messages=[UserMessage(content="Call both generic tools.")],
                tools=tools,
            )
        )

    calls = [event.tool_call for event in events if isinstance(event, ToolCallEndEvent)]
    assert calls == [
        ToolCall(id="call-alpha", name="tool_alpha", arguments={"value": "alpha"}),
        ToolCall(id="call-beta", name="tool_beta", arguments={"value": "beta"}),
    ]
    assert isinstance(events[-1], AssistantDoneEvent)
    assert events[-1].message.tool_calls == tuple(calls)


@pytest.mark.anyio
async def test_openai_compatible_provider_rejects_unnamed_tool_call_before_execution() -> None:
    requests: list[httpx.Request] = []
    executed = False
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            text=(
                'data: {"choices":[{"delta":{"tool_calls":[{"index":0,'
                '"id":"call-missing-name","function":{"arguments":'
                '"{\\"value\\":\\"input\\"}"}}]},"finish_reason":"tool_calls"}]}\n\n'
                "data: [DONE]\n\n"
            ),
            headers={"content-type": "text/event-stream"},
        )
    ...
    messages = [UserMessage(content="Call the generic tool.")]
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        provider = OpenAICompatibleProvider(
            OpenAICompatibleConfig(api_key="test-key", base_url="https://example.test/v1"),
            client=client,
            ...
            run_agent_loop(
                ...
                tools=[
                    AgentTool(
                        name="tool_alpha",
                        label="tool_alpha",
                        description="First generic tool.",
    ...
    assert not any(event.type.startswith("tool_execution") for event in events)
    ...
    assert error.error_message == (
        "OpenAI-compatible provider protocol error: tool call at index 0 is missing a function name"
    )
    assert error.diagnostics is not None
    assert error.diagnostics[0].details == {
...
async def test_openai_codex_provider_formats_request_and_streams_text() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert payload["stream"] is True
        assert payload["instructions"] == "You are Xoder."
    ...
    assert request.headers["originator"] == "xoder"
    assert request.headers["user-agent"].startswith("xoder (")
...
async def test_anthropic_provider_formats_request_and_streams_text() -> None:
    ...
    assert payload["stream"] is True
    assert payload["system"] == "You are Xoder."
...
def test_use_responses_api_routes_only_restricted_models() -> None:
    from xoder_ai.openai_compatible import _use_responses_api
...
@pytest.mark.anyio
async def test_responses_api_rejects_unnamed_tool_call() -> None:
    def handler(_request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200,
            text=(
                'data: {"type":"response.output_item.added","output_index":2,'
                '"item":{"id":"fc_missing","type":"function_call",'
                '"call_id":"call_missing","name":"","arguments":"{}"}}\n\n'
                'data: {"type":"response.completed","response":{"status":"completed"}}\n\n'
            ),
            headers={"content-type": "text/event-stream"},
        )
    ...
    assert isinstance(events[-1], AssistantErrorEvent)
    assert events[-1].error.tool_calls == ()
    assert events[-1].error.error_message == (
        "OpenAI-compatible provider protocol error: tool call at index 2 is missing a function name"
    )
