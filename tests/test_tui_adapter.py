...
from xoder_coding.events import AutoRetryStartEvent, QueueUpdateEvent
from xoder_coding.skills import Skill, format_skill_invocation
from xoder_coding.tui import TuiEventAdapter, TuiState
from xoder_coding.tui.state import format_tool_call_block, format_tool_result_block
...
def test_tui_adapter_builds_user_and_compact_skill_items() -> None:
    ...
    adapter.apply(MessageEndEvent(message=UserMessage(content="Hello Xoder")))
    adapter.apply(
...
def test_tui_adapter_renders_skill_file_reads_with_skill_style() -> None:
    ...
    adapter.apply(
        ToolExecutionStartEvent(
            tool_call_id="call-1",
            tool_name="read",
            args={"path": "/workspace/.xoder/skills/review.md"},
        )
    )
    adapter.apply(
        ToolExecutionEndEvent(
...
def test_tool_formatters_keep_human_readable_output() -> None:
    from xoder_agent import ToolCall
