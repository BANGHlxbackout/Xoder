...
from xoder_coding.provider_catalog import ModelCostTier
...
def test_load_provider_settings_accepts_provider_preferences_with_user_catalog(
    ...
    xoder_home.mkdir()
    (xoder_home / "catalog.toml").write_text(
        """
schema_version = 1
...
""".strip(),
    ...
    (xoder_home / "providers.json").write_text(
        json.dumps(
            {
    ...
    settings = load_provider_settings(XoderPaths(home=xoder_home))
...
def test_save_and_load_provider_settings_round_trip(tmp_path: Path) -> None:
    ...
    assert path == tmp_path / ".xoder" / "providers.json"
...
def test_kimi_k3_maps_xhigh_thinking_to_max(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = load_provider_settings(XoderPaths(home=Path("/missing")))
...
""".strip(),
    ...
    store = FileCredentialStore(xoder_home / "credentials.json")
