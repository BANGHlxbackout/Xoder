...
from xoder_coding.catalog_loader import (
    CatalogError,
    builtin_catalog,
...
from xoder_coding.provider_config import load_provider_settings
...
def _write_user_catalog(xoder_home: Path, body: str) -> XoderPaths:
    paths = XoderPaths(home=xoder_home)
    xoder_home.mkdir(parents=True, exist_ok=True)
...
def test_user_catalog_adds_new_provider(tmp_path: Path) -> None:
    paths = _write_user_catalog(tmp_path / ".xoder", VALID_PROVIDER)
...
def test_user_catalog_rejects_unknown_keys(tmp_path: Path) -> None:
    paths = _write_user_catalog(tmp_path / ".xoder", VALID_PROVIDER.replace("docs_url", "docs_ur1"))
...
def test_user_catalog_rejects_empty_and_coerced_values(
    ...
    paths = _write_user_catalog(tmp_path / ".xoder", body)
...
def test_user_catalog_rejects_bad_kind(tmp_path: Path) -> None:
    paths = _write_user_catalog(
        tmp_path / ".xoder", VALID_PROVIDER.replace("openai-compatible", "grpc")
    )
...
def test_user_catalog_rejects_malformed_toml(tmp_path: Path) -> None:
    paths = _write_user_catalog(tmp_path / ".xoder", "[[providers]\nname =")
...
def test_user_catalog_provider_appears_with_existing_settings_file(tmp_path: Path) -> None:
    paths = _write_user_catalog(tmp_path / ".xoder", VALID_PROVIDER)
    (tmp_path / ".xoder" / "providers.json").write_text(
