"""HTTP client helpers shared by Xoder network integrations."""
...
def normalize_proxy_url(proxy_url: str) -> str:
    ...
    request. Treat the generic form as SOCKS5 so Xoder can honor these proxy
...
def create_async_client(**kwargs: Any) -> httpx.AsyncClient:
    """Create an ``httpx.AsyncClient`` with Xoder's proxy normalization applied."""
...
def get_json(url: str, *, timeout: float, follow_redirects: bool = False) -> dict[str, object]:
    """Fetch a JSON object with Xoder's proxy normalization applied."""
