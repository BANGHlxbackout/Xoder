...
from xoder_agent.tools import AgentTool, ToolCall
from xoder_agent.types import JSONValue
from xoder_ai._provider_events import (
    ProviderErrorEvent,
    ProviderEvent,
...
from xoder_ai.env import OpenAICompatibleConfig
from xoder_ai.events import AssistantMessageEvent
from xoder_ai.http import create_async_client
from xoder_ai.http_errors import provider_http_error_message
from xoder_ai.provider import CancellationToken
from xoder_ai.retry import provider_retry_event, retry_delay_seconds, wait_for_retry
from xoder_ai.stream import canonicalize_provider_stream
...
class _ChatStreamParser:
    ...
    def finalize(self) -> list[ProviderEvent]:
        for index, builder in sorted(self._tool_call_builders.items()):
            if not builder.name:
                self.fatal = True
                return [_missing_tool_name_error(index)]
...
class _ResponsesStreamParser:
    ...
    def finalize(self) -> list[ProviderEvent]:
        ...
        for builder in builders:
            if not builder.name:
                self.fatal = True
                return [_missing_tool_name_error(builder.output_index)]
...
def _missing_tool_name_error(index: int) -> ProviderErrorEvent:
    """Return a terminal error for an incomplete streamed function call."""
    return ProviderErrorEvent(
        message=(
            "OpenAI-compatible provider protocol error: "
...
def _parse_chunk_usage(raw: Mapping[str, Any]) -> Usage:
    ...
    unset (None) because Xoder has no per-model pricing table.
    """
...
def _usage_from_responses_event(chunk: Mapping[str, Any]) -> Usage | None:
    ...
    cost is left unset because Xoder has no per-model pricing table.
    """
    response = chunk.get("response")
