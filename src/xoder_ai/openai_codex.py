...
from xoder_ai.model_limits import RuntimeModelLimits
...
class _ToolCallBuilder:
    ...
    def build(self) -> ToolCall:
        """Build a complete Xoder tool call."""
...
def _usage_from_response(event: Mapping[str, Any]) -> Usage | None:
    ...
    Cost is left unset (None) because Xoder has no per-model pricing table.
    """
    response = event.get("response")
    if not isinstance(response, Mapping):
        return None
    ...
    *,
    ...
    headers = {
        **dict(configured_headers or {}),
        ...
        "User-Agent": f"xoder ({system()} {release()}; {machine()})",
