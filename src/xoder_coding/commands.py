"""Slash command registry for Xoder coding sessions."""
...
from xoder_coding.provider_catalog import BUILTIN_PROVIDER_CATALOG, builtin_provider_entry
from xoder_coding.reload import CodingReloadSummary, ReloadCategorySummary
from xoder_coding.resources import ResourceDiagnostic
...
from xoder_coding.thinking import normalize_thinking_level
...
def create_default_command_registry() -> CommandRegistry:
    """Create Xoder's built-in slash command registry."""
    registry = CommandRegistry()
    registry.register(
        SlashCommand(
            name="quit",
...
def _theme_command(context: CommandContext) -> CommandResult:
    ...
    # Imported lazily so importing this module never pulls in `xoder_coding.tui`
    # (whose package __init__ imports Textual) until /theme actually executes.
    from xoder_coding.tui.themes import available_tui_theme_names
