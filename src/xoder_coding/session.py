...
from xoder_agent.events import AgentEndEvent, MessageEndEvent, ToolExecutionEndEvent
from xoder_agent.harness import AgentHarness, AgentHarnessConfig, QueuedMessages
...
from xoder_agent.provider import ModelProvider
from xoder_agent.provider_events import AssistantDoneEvent, AssistantErrorEvent, TextDeltaEvent
...
from xoder_agent.session.entries import SessionEntry
from xoder_agent.session.jsonl import entry_to_json_line
from xoder_agent.session.tree import SessionTreeError, path_to_entry
...
from xoder_coding.branch_summary import summarize_branch_messages_with_model
from xoder_coding.commands import CommandRegistry, CommandResult, create_default_command_registry
from xoder_coding.context import discover_project_context_with_diagnostics
from xoder_coding.context_window import (
...
from xoder_coding.credentials import FileCredentialStore, credentials_path
from xoder_coding.diagnostics import (
    AgentCallDiagnosticContext,
    AgentCallDiagnosticLogger,
    new_agent_call_run_id,
)
from xoder_coding.events import (
    AgentSettledEvent,
    AutoRetryEndEvent,
    AutoRetryStartEvent,
    CodingSessionEvent,
...
from xoder_coding.extensions.runtime import ExtensionRuntime
from xoder_coding.paths import XoderPaths
from xoder_coding.prompt_templates import (
    PromptTemplate,
...
from xoder_coding.provider_runtime import ClosableModelProvider, create_model_provider
from xoder_coding.reload import CodingReloadSummary, ReloadCategorySummary
from xoder_coding.resources import (
    ResourceDiagnostic,
    ResourceError,
    XoderResourcePaths,
    resource_paths_with_cwd,
)
from xoder_coding.session_export import (
    export_session_artifact,
    normalize_export_format,
)
from xoder_coding.session_manager import SessionManager
from xoder_coding.session_stats import SessionStats, calculate_session_stats
from xoder_coding.skills import Skill, expand_skill_command, load_skills_with_diagnostics
from xoder_coding.system_prompt import (
    BuildSystemPromptOptions,
    ProjectContextFile,
    build_system_prompt,
)
from xoder_coding.thinking import (
...
from xoder_coding.tools import create_bash_tool, create_coding_tools
...
TREE_RUNNING_MESSAGE = "Xoder is still working. Press Escape to interrupt before using /tree."
...
class SessionResources:
    """Xoder-owned resources loaded around a coding session."""
...
class CodingSessionConfig:
    ...
    resource_paths: XoderResourcePaths | None = None
...
class CodingSession:
    """Xoder's coding-agent environment wrapper.
    ...
    def available_providers(self) -> tuple[str, ...]:
        """Return provider names Xoder can call with available credentials."""
    ...
    def available_model_choices(self) -> tuple[ModelChoice, ...]:
        """Return provider/model choices Xoder can call with available credentials."""
        if self._provider_settings is None:
...
def _resolve_export_destination(
    ...
    *,
    ...
    if destination is None:
        return cwd / f"xoder-session.{format}"

    resolved = destination if destination.is_absolute() else cwd / destination
    if resolved.suffix:
        return resolved
    return resolved / f"xoder-session.{format}"
...
def _session_export_title(session: CodingSession) -> str:
    ...
    if manager is not None and session_id is not None:
        ...
        if record is not None and record.title:
            return record.title
    return f"Xoder session {session_id}" if session_id is not None else "Xoder Session Export"
...
def _load_session_resources(
    resource_paths: XoderResourcePaths,
...
def default_session_path(cwd: Path) -> Path:
    """Return Xoder's default user-home session path for a project cwd."""
    return XoderPaths().default_session_path(cwd)
