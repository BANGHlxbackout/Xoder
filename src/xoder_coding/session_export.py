...
def export_session_html(
    ...
    output_path: Path,
    *,
    title: str = "Xoder Session Export",
...
<body>
  <header>
    <div class="header-top">
      <p class="eyebrow">Xoder session export</p>
      <button
        type="button"
        class="theme-toggle"
        id="themeToggle"
  ...
  <script>
    (function () {{
      ...
      var stored = null;
      try {{
        stored = window.localStorage.getItem("xoder-session-export-theme");
      }} catch (err) {{
        stored = null;
      }}
      ...
      toggle.addEventListener("click", function () {{
        ...
        try {{
          window.localStorage.setItem("xoder-session-export-theme", next);
        }} catch (err) {{
