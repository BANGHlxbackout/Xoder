...
from xoder_coding.resources import ResourceDiagnostic, XoderResourcePaths

EXTENSION_ENTRY_ATTRIBUTE = "setup"
_MODULE_NAME_PREFIX = "xoder_extension"
...
def _manifest_entries(
    ...
    """Resolve entry files declared in `<dir>/pyproject.toml` under `[tool.xoder]`.
    ...
    use package.json manifest"); `tool.xoder.extensions` is the Python-shaped
    ...
    xoder_table = tool_table.get("xoder") if isinstance(tool_table, dict) else None
    declared = xoder_table.get("extensions") if isinstance(xoder_table, dict) else None
    if declared is None:
        return []
    if not isinstance(declared, list) or not all(isinstance(item, str) for item in declared):
        diagnostics.append(
            ResourceDiagnostic(
                kind="extension",
                path=manifest,
                message="`tool.xoder.extensions` must be a list of file paths",
                severity="error",
            )
        )
        return []
    ...
    if not callable(setup):
        ...
        return None, [
            _error_diagnostic(
                ...
                f"`{EXTENSION_ENTRY_ATTRIBUTE}(xoder)` entry point",
            )
        ]
