"""Best-effort PyPI update checks for the Xoder CLI."""
...
from xoder_ai.http import get_json
from xoder_coding.paths import XoderPaths

PYPI_PACKAGE_NAME = "xoder-ai"
...
UPDATE_CHECK_ENV_DISABLE = "XODER_NO_UPDATE_CHECK"
...
class UpdateNotice:
    ...
    def message(self) -> str:
        ...
        return (
            f"Xoder {self.latest_version} is available (installed: {self.current_version}). "
...
class ReleaseNotesEntry:
    """Structured release notes for one Xoder version."""
...
class ReleaseNotesNotice:
    """Release notes shown once after the installed Xoder version changes."""
    ...
    def message(self) -> str:
        ...
        return f"Xoder updated to {self.current_version}\n\n{body}"
...
def startup_update_notice(
    ...
    *,
    ...
    """Return an update notice when PyPI has a newer stable Xoder release.
...
def startup_release_notes_notice(
    ...
    *,
    ...
    """Return release notes once after Xoder starts with a newer installed version.
...
def fetch_latest_pypi_version(*, fetcher: Fetcher | None = None) -> str | None:
    """Fetch the latest stable Xoder version from PyPI."""
...
def default_update_check_cache_path(paths: XoderPaths | None = None) -> Path:
    """Return the on-disk cache path for startup update checks."""
    return (paths or XoderPaths()).home / "cache" / "update-check.json"


def default_release_notes_state_path(paths: XoderPaths | None = None) -> Path:
    """Return the on-disk state path for one-time release notes."""
    return (paths or XoderPaths()).home / "cache" / RELEASE_NOTES_STATE_FILENAME
