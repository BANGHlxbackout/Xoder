"""Local credential storage for Xoder provider credentials."""
...
class CredentialStoreError(ValueError):
    """Raised when Xoder credential storage cannot be read or written."""


@dataclass(frozen=True, slots=True)
class OAuthCredential:
    """Refreshable OAuth credential persisted under Xoder home.
...
class ApiKeyCredential:
    """API-key credential persisted under Xoder home."""
...
class FileCredentialStore:
    """Small JSON-backed provider credential store under Xoder home."""
    ...
    def _load(self) -> dict[str, StoredCredential]:
        ...
        if not isinstance(raw, dict):
            raise CredentialStoreError("Xoder credentials must be a JSON object")
        credentials: dict[str, StoredCredential] = {}
        for key, value in raw.items():
            if not isinstance(key, str):
                raise CredentialStoreError("Xoder credential names must be strings")
...
def credentials_path(paths: XoderPaths | None = None) -> Path:
    """Return Xoder's local provider credential path."""
    return (paths or XoderPaths()).home / "credentials.json"
...
def _credential_from_json(value: object) -> StoredCredential:
    ...
    if not isinstance(value, dict):
        raise CredentialStoreError("Xoder credential values must be strings or objects")

    credential_type = value.get("type")
    if credential_type not in {"api_key", "oauth"}:
        raise CredentialStoreError("Xoder credential object type must be api_key or oauth")
    ...
    if not isinstance(expires, int) or isinstance(expires, bool) or expires <= 0:
        raise CredentialStoreError("Xoder oauth credential expires must be a positive integer")
    account_id = value.get("account_id")
    if account_id is not None and (not isinstance(account_id, str) or not account_id.strip()):
        raise CredentialStoreError("Xoder oauth credential account_id must be a non-empty string")
    metadata = value.get("metadata", {})
    if not isinstance(metadata, dict):
        raise CredentialStoreError("Xoder oauth credential metadata must be an object")
...
def _validate_oauth_metadata(metadata: dict[Any, Any]) -> None:
    for key, value in metadata.items():
        if not isinstance(key, str) or not key.strip():
            raise CredentialStoreError("Xoder oauth credential metadata keys must be strings")
        if not _is_json_value(value):
            raise CredentialStoreError("Xoder oauth credential metadata values must be JSON values")
...
def _string_field(
    ...
    if not isinstance(field, str) or not field.strip():
        raise CredentialStoreError(
            f"Xoder {credential_type} credential field must be a non-empty string: {field_name}"
        )
    return field.strip()
