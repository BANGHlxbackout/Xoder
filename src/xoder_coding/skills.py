...
def load_skills(paths: XoderResourcePaths | None = None) -> list[Skill]:
    """Load markdown skills from Xoder and `.agents` resource directories.
...
def parse_skill_invocation(text: str) -> SkillInvocation | None:
    """Parse Xoder's expanded skill invocation message format."""
    match = re.match(
...
def _load_skills_from_dir_with_diagnostics(
    ...
    # Xoder follows the Agent Skills spec everywhere: a skill is a directory
