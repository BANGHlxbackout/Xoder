"""Project instruction discovery for Xoder coding sessions."""
...
def discover_project_context(
    paths: XoderResourcePaths | None = None,
...
def discover_project_context_with_diagnostics(
    ...
    resource_paths = paths or XoderResourcePaths()
...
def _context_file_candidates(paths: XoderResourcePaths) -> tuple[Path, ...]:
    ...
    if paths.cwd is not None:
        ...
        xoder_paths = paths._paths()
        candidates.extend(
            [
                xoder_paths.project_xoder_dir(cwd) / "AGENTS.md",
                xoder_paths.project_agents_dir(cwd) / "AGENTS.md",
            ]
        )
