"""Xoder markdown resource path and frontmatter helpers."""
...
class ResourceError(ValueError):
    """Raised when Xoder resources are invalid or cannot be expanded."""
...
@dataclass(frozen=True, slots=True)
class XoderResourcePaths:
    """Filesystem locations for Xoder markdown resources.

    By default Xoder loads both Xoder-native resources and `.agents` resources from
    the user home directory. When a cwd is provided, project-local `.xoder` and
    `.agents` resources are loaded automatically as well.
    """

    root: Path = field(default_factory=lambda: Path.home() / ".xoder")
    cwd: Path | None = None
    ...
    def skills_dir(self) -> Path:
        """Return the primary Xoder skills directory."""
        return self.root / "skills"

    @property
    def prompts_dir(self) -> Path:
        """Return the primary Xoder prompt templates directory."""
    ...
    def skills_dirs(self) -> tuple[Path, ...]:
        ...
        """
        paths = self._paths()
        dirs = [xoder_builtin_skills_path(), self.skills_dir]
    ...
    def themes_dirs(self) -> tuple[Path, ...]:
        ...
        Themes are Xoder-specific, so unlike skills and prompts no ``.agents``
    ...
    def _paths(self) -> XoderPaths:
        agents_home = self.agents_root or Path.home() / ".agents"
        return self.paths or XoderPaths(home=self.root, agents_home=agents_home)
