"""Load Xoder's provider catalog from packaged and user TOML files."""
...
from xoder_coding.thinking import ThinkingLevel, ThinkingParameter
...
class CatalogError(ValueError):
    """Raised when an Xoder catalog file is invalid."""
...
def builtin_catalog_resource_text() -> str:
    ...
    return files("xoder_coding").joinpath("data/catalog.toml").read_text(encoding="utf-8")


@cache
def builtin_catalog() -> tuple[ProviderCatalogEntry, ...]:
    """Return Xoder's built-in provider catalog from the packaged data file."""
    return _entries_from_raw(_builtin_raw(), source="built-in catalog.toml")


def user_catalog_path(paths: XoderPaths | None = None) -> Path:
    """Return the user-level catalog overlay path."""
    return (paths or XoderPaths()).home / USER_CATALOG_FILENAME


def effective_catalog(paths: XoderPaths | None = None) -> tuple[ProviderCatalogEntry, ...]:
    """Return the builtin catalog with the user's ~/.xoder/catalog.toml overlaid."""
