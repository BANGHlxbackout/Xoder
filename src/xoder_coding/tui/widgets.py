"""Small Textual widgets for Xoder's interactive TUI."""
...
from xoder_coding.skills import Skill
from xoder_coding.system_prompt import ProjectContextFile
from xoder_coding.tui.autocomplete import CompletionState
from xoder_coding.tui.config import XODER_DARK_THEME, TuiRoleStyle, TuiTheme
from xoder_coding.tui.state import ChatItem, TuiState
from xoder_coding.version import current_version

XODER_SIDEBAR_LOGO = "Xoder"
...
class XoderMarkdownBlock(MarkdownBlock):
    """Markdown block that applies Xoder's themed inline link color."""
    ...
    def _token_to_content(self, token: Any) -> Any:
        ...
        if not isinstance(markdown, ThemedMarkdownWidget):
            return content
        link_style = TextualStyle.parse(markdown.xoder_link_style)
        spans = []
...
class ThemedMarkdownWidget(TextualMarkdown):
    """Textual Markdown widget reserved for Xoder transcript streaming."""

    BLOCKS = {**TextualMarkdown.BLOCKS, "paragraph_open": XoderMarkdownBlock}
    ...
    ThemedMarkdownWidget MarkdownH6 {
        color: $xoder-markdown-highlight;
        content-align: left middle;
        text-style: bold;
    }

    ThemedMarkdownWidget MarkdownBlock > .code_inline {
        color: $xoder-markdown-inline-code !important;
        background: transparent !important;
    }

    ThemedMarkdownWidget MarkdownBullet {
        color: $xoder-markdown-bullet;
    }

    ThemedMarkdownWidget MarkdownFence {
        background: $xoder-markdown-code-block-background;
        overflow-x: auto;
    ...
    ThemedMarkdownWidget MarkdownTableContent {
        keyline: thin $xoder-markdown-table-border;
    }

    ThemedMarkdownWidget MarkdownTableContent > .header {
        color: $xoder-markdown-table-header;
        text-style: bold;
    }
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.xoder_link_style = theme.markdown_link
...
class TranscriptView(VerticalScroll):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._render_theme: TuiTheme = XODER_DARK_THEME
...
def _sidebar_brand(*, theme: TuiTheme) -> RenderableType:
    ...
    brand.append(XODER_SIDEBAR_LOGO)
...
class ThemedCodeBlock(CodeBlock):
    """Rich Markdown code block with Xoder's themed background color."""
...
class ThemedMarkdown(Markdown):
    """Markdown renderer with Xoder's softer heading/accent colors."""
