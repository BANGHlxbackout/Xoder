"""Helpers for updating the terminal window/tab title from Xoder's TUI."""
...
def terminal_title_supported(
    *,
    ...
    """Return whether Xoder should emit OSC title sequences in this process."""
    env = os.environ if environ is None else environ
    if env.get("XODER_TERMINAL_TITLE", "").lower() in {"0", "false", "no", "off"}:
        return False
    ...
    if env.get("TERM", "") == "dumb":
        return False
    return not (env.get("CI", "") and env.get("XODER_TERMINAL_TITLE", "").lower() != "1")
...
def build_terminal_title(
    ...
    *,
    ...
    """Return Xoder's terminal tab title for the current session/running state."""
    title = sanitize_terminal_title(session_title)
    title = (
        XODER_TITLE_MARK
        if not title or title.lower() == "untitled session"
        else f"{XODER_TITLE_MARK} | {title}"
    )
    if not running:
        return title
...
class TerminalTitleController:
    ...
    def __init__(
        ...
        *,
        ...
        stream: TextIO | None = None,
        environ: Mapping[str, str] | None = None,
        exit_title: str = XODER_TITLE_MARK,
    ) -> None:
    ...
    def update(self, session_title: str | None, *, running: bool, frame: int = 0) -> None:
        """Write the current Xoder title if it differs from the last emitted title."""
        if not self.enabled:
            return
    ...
    def restore(self) -> None:
        """Leave the terminal title in a neutral idle Xoder state on shutdown."""
        if not self.enabled:
            return
