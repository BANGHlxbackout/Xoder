"""Minimal Textual app for Xoder coding sessions."""
...
from xoder_agent.provider import CancellationToken
...
from xoder_agent.tools import AgentTool
from xoder_agent.types import JSONValue
from xoder_coding.catalog_loader import save_user_catalog_entries
from xoder_coding.commands import (
...
from xoder_coding.events import AutoRetryStartEvent, CodingSessionEvent, QueueUpdateEvent
from xoder_coding.extensions.api import (
    KeyInterceptor,
    MainViewFactory,
    MainViewHandle,
    Placement,
...
from xoder_coding.oauth import login_openai_codex
from xoder_coding.oauth_registry import get_oauth_provider, oauth_provider_ids
from xoder_coding.oauth_types import (
    OAuthAuthInfo,
    OAuthDeviceCodeInfo,
    OAuthLoginCallbacks,
    OAuthPrompt,
...
from xoder_coding.provider_catalog import (
...
from xoder_coding.provider_runtime import create_model_provider
...
from xoder_coding.session_manager import CodingSessionRecord, SessionManager
from xoder_coding.shell_config import load_shell_settings
from xoder_coding.tui.adapter import TuiEventAdapter
from xoder_coding.tui.autocomplete import (
    CompletionItem,
    CompletionOption,
    CompletionState,
...
from xoder_coding.tui.state import TuiState, format_terminal_command_result_block
...
PROMPT_PLACEHOLDER = "Ask Xoder…  Enter submits, Shift+Enter inserts a newline"
...
class _TuiExtensionUiBridge:
    ...
    def __init__(self, app: XoderTuiApp) -> None:
        self._app = app
    ...
    def register_key_interceptor(self, handler: KeyInterceptor) -> Callable[[], None]:
        ...
        ``XoderTuiApp.on_event`` before Textual's app-level priority bindings and
        before the focused widget receives the key, so it can own navigation
        keys (``up``/``down``/``tab``/…) that Xoder otherwise binds with
...
class _MainViewHandle:
    ...
    def __init__(self, app: XoderTuiApp, result: asyncio.Future[object | None]) -> None:
        self._app = app
        self._open = True
...
class PromptInput(TextArea):
    ...
    async def on_key(self, event: Key) -> None:
        ...
        :meth:`XoderTuiApp.on_event` (pre-dispatch, before app-level priority
...
#: (Xoder binds it to ``clear_prompt``, but it is the terminal-standard
#: SIGINT/interrupt reflex users hit to bail). NOT reserved: escape/enter/
#: arrows/tab/left/right — those are load-bearing for the xoder-subagents
#: extension and must stay interceptable. This is Xoder's counterpart to Pi's
...
class XoderTuiApp(App[None]):
    ...
    Screen {
        ...
        color: $xoder-screen-text;
    }

    Toast {
        background: $xoder-chrome-background;
        color: $xoder-chrome-text;
    }
    ...
    XoderTuiApp.-hide-sidebar #sidebar {
        display: none;
    }

    XoderTuiApp.-hide-sidebar #main-pane {
        padding-left: 1;
    }

    XoderTuiApp.-sidebar-right #sidebar {
        dock: right;
    }
    ...
    #transcript {
        height: 1fr;
        border: none;
        background: $xoder-transcript-background;
        padding: 0 0 0 2;
        overflow-x: auto;
    ...
    #autocomplete {
        ...
        margin: 0 1 1 1;
        padding: 0 1;
        background: $xoder-autocomplete-background;
        color: $xoder-screen-text;
        border: tall $xoder-border;
        overflow-y: auto;
    }
    ...
    #session-picker-search {
        ...
        color: $xoder-prompt-text;
        border: tall $xoder-prompt-border;
    }
    ...
    ListView > ListItem.-highlight {
        background: $xoder-highlight-background;
        color: $xoder-highlight-text;
    }
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._register_xoder_textual_themes()
    ...
    def _register_xoder_textual_themes(self) -> None:
        """Register Xoder themes with Textual's theme system.

        Textual exposes its own theme menu and command palette entries. Registering
        Xoder's built-in themes there makes those controls update the same theme as
        `/theme` instead of changing only Textual's chrome.
        """
        self._registered_themes.clear()
        for theme_name in available_tui_theme_names():
            self.register_theme(_textual_theme_for_xoder_theme(theme_name))

    def _watch_theme(self, theme_name: str) -> None:
        """Keep Textual theme changes synchronized with Xoder's durable TUI theme."""
        super()._watch_theme(theme_name)
        if theme_name not in available_tui_theme_names():
            return
        if getattr(self, "_applying_settings_theme", False):
            return
        xoder_theme: TuiThemeName = theme_name
        if self.tui_settings.theme == xoder_theme:
            return
        self._replace_tui_settings(theme=xoder_theme)
        save_tui_settings(self.tui_settings)

    def get_theme_variable_defaults(self) -> dict[str, str]:
        """Return Xoder-specific CSS variables for the selected TUI theme."""
        ...
        with Horizontal(id="workspace"):
            ...
            with Vertical(id="main-pane"):
                ...
                with Horizontal(id="prompt-row"):
                    yield Static("X", id="prompt-prefix")
                    yield PromptInput(
    ...
    async def on_event(self, event: events.Event) -> None:
        ...
        the earliest point in key processing — before Xoder's app-level priority
    ...
    def action_open_session_picker(self) -> None:
        ...
        if self.state.running:
            self._notify("Xoder is already working. Press Escape to cancel.")
            return
...
def _render_activity_indicator(theme: TuiTheme, *, frame: int, running: bool) -> Text:
    """Render the prompt prefix, turning Xoder into a moving square while running."""
    if not running:
        return Text("X", style=f"bold {theme.accent}")
...
def _textual_theme_for_xoder_theme(theme_name: TuiThemeName) -> Theme:
    """Map an Xoder theme to Textual's native theme type."""
...
def _theme_css_variables(theme: TuiTheme) -> dict[str, str]:
    """Return Textual CSS variables for a resolved Xoder theme."""
    return {
        "xoder-screen-background": theme.screen_background,
        "xoder-screen-text": theme.screen_text,
        "xoder-chrome-background": theme.chrome_background,
        "xoder-chrome-text": theme.chrome_text,
        "xoder-muted-text": theme.muted_text,
        "xoder-sidebar-background": theme.sidebar_background,
        "xoder-border": theme.border,
        "xoder-transcript-background": theme.transcript_background,
        ...
        "xoder-highlight-text": theme.highlight_text,
        "xoder-markdown-highlight": theme.markdown_heading,
        "xoder-markdown-table-header": theme.markdown_table_header,
        "xoder-markdown-table-border": theme.markdown_table_border,
        "xoder-markdown-inline-code": theme.markdown_inline_code,
        "xoder-markdown-code-block-background": theme.markdown_code_block_background,
        "xoder-markdown-link": theme.markdown_link,
        "xoder-markdown-bullet": theme.markdown_bullet,
    ...
    *,
    ...
    try:
        ...
        custom_themes, theme_diagnostics = load_custom_tui_themes(
            XoderResourcePaths(cwd=record.cwd).themes_dirs
        )
