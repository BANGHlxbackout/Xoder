"""JSON-defined TUI themes for Xoder.
...
``~/.xoder/themes/*.json`` and ``<project>/.xoder/themes/*.json``.
"""
...
@dataclass(frozen=True, slots=True)
class TuiTheme:
    """Resolved visual theme for Xoder's built-in Textual frontend."""
...
XODER_DARK_THEME = _load_builtin_theme("xoder-dark.json")
XODER_LIGHT_THEME = _load_builtin_theme("xoder-light.json")
HIGH_CONTRAST_THEME = _load_builtin_theme("high-contrast.json")

_BUILTIN_THEMES: dict[str, TuiTheme] = {
    theme.name: theme for theme in (XODER_DARK_THEME, XODER_LIGHT_THEME, HIGH_CONTRAST_THEME)
}
...
def get_tui_theme(name: TuiThemeName = "xoder-dark") -> TuiTheme:
