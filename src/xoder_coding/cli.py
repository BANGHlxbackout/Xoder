"""Command-line entry point for Xoder."""
...
from xoder_agent.session import JsonlSessionStorage, SessionEntry, SessionStorage
...
from xoder_coding.commands import format_reload_summary
from xoder_coding.credentials import FileCredentialStore
from xoder_coding.extensions import StderrUiBridge
...
from xoder_coding.rendering import PrintOutputMode, create_event_renderer
...
from xoder_coding.tui import run_tui_app
from xoder_coding.update_check import (
    UpdateNotice,
    startup_release_notes_notice,
    startup_update_notice,
)
from xoder_coding.version import current_version as _current_version
...
app = typer.Typer(
    name="xoder",
    help="Xoder personal agent harness.",
...
def setup_command(
    *,
    ...
    if provider.api_key_env not in environ:
        typer.echo(f"Set {provider.api_key_env} before running Xoder with this provider.", err=True)
...
def main(
    ...
    setup_base_url: Annotated[
        str,
        typer.Option("--base-url", help="OpenAI-compatible base URL for `xoder setup`."),
    ] = DEFAULT_OPENAI_COMPATIBLE_BASE_URL,
    setup_api_key_env: Annotated[
        str,
        typer.Option("--api-key-env", help="API key environment variable for `xoder setup`."),
    ] = "OPENAI_API_KEY",
    setup_timeout_seconds: Annotated[
        float,
        typer.Option(
            "--timeout-seconds",
            help="HTTP timeout in seconds for `xoder setup` provider requests.",
        ),
    ] = DEFAULT_OPENAI_COMPATIBLE_TIMEOUT_SECONDS,
    setup_max_retries: Annotated[
        int,
        typer.Option("--max-retries", help="Provider retry count for `xoder setup`."),
    ] = DEFAULT_OPENAI_COMPATIBLE_MAX_RETRIES,
    setup_max_retry_delay_seconds: Annotated[
        float,
        typer.Option(
            "--max-retry-delay-seconds",
            help="Provider retry delay in seconds for `xoder setup`.",
        ),
    ] = DEFAULT_OPENAI_COMPATIBLE_MAX_RETRY_DELAY_SECONDS,
    ...
    project_extensions: Annotated[
        ...
        typer.Option(
            "--project-extensions",
            help="Also load project .xoder/extensions (runs project-supplied code at startup).",
        ),
    ] = False,
    version: Annotated[
        bool,
        typer.Option("--version", help="Show Xoder's version and exit."),
    ] = False,
) -> None:
    """Run the Xoder CLI."""
    current_version = _current_version()
    if version:
        typer.echo(f"xoder {current_version}")
        raise typer.Exit()
...
def _parse_export_cli_args(args: list[str]) -> tuple[str, Path | None, str | None]:
    if not args:
        raise RuntimeError(
            "Usage: xoder export <session-id-or-jsonl> [--format html|jsonl] [output]"
        )
    session_ref = args[0]
    output_path: Path | None = None
    export_format: str | None = None
...
def _resolve_export_destination(
    ...
    *,
    ...
    if output_path is None:
        return Path.cwd() / f"xoder-session.{format}"
    if output_path.suffix:
        return output_path
    return output_path / f"xoder-session.{format}"
...
def _resolve_export_source(
    ...
    if candidate_path.exists():
        ...
        return candidate_path, f"Xoder session {candidate_path.stem}"
    ...
    title = record.title or f"Xoder session {record.id}"
