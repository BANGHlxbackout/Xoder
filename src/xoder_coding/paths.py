"""Canonical filesystem paths for Xoder user and project data."""
...
class XoderPaths:
    """Resolved Xoder filesystem locations.

    Xoder keeps durable application data under the user's home directory while also
    loading project-local resources from the active working directory.
    """

    home: Path = field(default_factory=lambda: Path.home() / ".xoder")
    ...
    def logs_dir(self) -> Path:
        """Return Xoder's user-level diagnostic log directory."""
    ...
    def user_skills_dir(self) -> Path:
        """Return Xoder's user-level skills directory."""
        return self.home / "skills"

    @property
    def user_prompts_dir(self) -> Path:
        """Return Xoder's user-level prompt templates directory."""
        return self.home / "prompts"

    @property
    def user_themes_dir(self) -> Path:
        """Return Xoder's user-level TUI themes directory."""
    ...
    def project_xoder_dir(self, cwd: Path) -> Path:
        """Return the project-local Xoder resource directory."""
        return cwd / ".xoder"
    ...
    def project_skills_dir(self, cwd: Path) -> Path:
        """Return the project-local Xoder skills directory."""
        return self.project_xoder_dir(cwd) / "skills"

    def project_prompts_dir(self, cwd: Path) -> Path:
        """Return the project-local Xoder prompt templates directory."""
        return self.project_xoder_dir(cwd) / "prompts"

    def project_themes_dir(self, cwd: Path) -> Path:
        """Return the project-local Xoder TUI themes directory."""
        return self.project_xoder_dir(cwd) / "themes"
