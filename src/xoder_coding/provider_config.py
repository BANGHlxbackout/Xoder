"""Durable provider configuration for Xoder coding sessions."""
...
from xoder_ai.openai_codex import DEFAULT_OPENAI_CODEX_BASE_URL
from xoder_coding.catalog_loader import effective_catalog, save_user_catalog_entries
from xoder_coding.credentials import FileCredentialStore, credentials_path
from xoder_coding.oauth_registry import get_oauth_provider
...
class ProviderConfigError(ValueError):
    """Raised when Xoder provider configuration is invalid."""
...
class ProviderSettings:
    """Xoder provider settings loaded from Xoder home."""
...
class ProviderSelection:
    """Resolved provider/model selection for an Xoder run."""
...
def builtin_provider_configs() -> tuple[ProviderConfig, ...]:
    """Return Xoder's built-in provider configs."""
    return tuple(
...
def default_openai_provider_config() -> OpenAICompatibleProviderConfig:
    """Return Xoder's default OpenAI-compatible provider entry."""
...
def provider_settings_path(paths: XoderPaths | None = None) -> Path:
    """Return the durable provider settings path."""
    return (paths or XoderPaths()).home / "providers.json"
...
def load_provider_settings(paths: XoderPaths | None = None) -> ProviderSettings:
    ...
    resolved_paths = paths or XoderPaths()
...
def save_provider_settings(settings: ProviderSettings, paths: XoderPaths | None = None) -> Path:
...
def _effective_provider_configs(paths: XoderPaths | None = None) -> tuple[ProviderConfig, ...]:
    ...
    *,
    ...
    for name, preference_data in value.items():
        ...
        if provider_name not in catalog_configs:
            ...
            # preference behind. Ignore it so one stale entry cannot prevent Xoder
            # from starting or running `xoder setup` to register it again.
            continue
        providers.append(
    ...
    *,
    ...
    if provider.thinking_levels is None:
        ...
        if isinstance(provider, OpenAICodexProviderConfig):
            return (
                "OpenAI Codex subscription can stream reasoning output, but Xoder does "
...
def provider_has_usable_credentials(
    ...
    *,
    ...
    """Return whether Xoder can attempt calls for this provider without prompting setup."""
...
def _reasoning_effort_from_provider(
    ...
    *,
    ...
    if provider.name == "huggingface" and normalized == "minimal":
        ...
        # for reasoning_effort, but rejects Pi/Xoder's "minimal" label.
        return "low"
