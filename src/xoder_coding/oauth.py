...
from xoder_coding.credentials import OAuthCredential
...
class OpenAICodexOAuthProvider:
    ...
    async def login(self, callbacks: OAuthLoginCallbacks) -> OAuthCredential:
        if callbacks.method == "device_code":
            raise OAuthError("OpenAI Codex device-code login is not implemented in Xoder yet")
...
async def _start_local_oauth_server(
    ...
    *,
    ...
    host = environ.get("XODER_OAUTH_CALLBACK_HOST", "127.0.0.1")
...
def _oauth_html(message: str) -> str:
    ...
    return f'<!doctype html><meta charset="utf-8"><title>Xoder OAuth</title><p>{escaped}</p>'
