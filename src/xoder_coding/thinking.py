"""Thinking-mode primitives for Xoder coding sessions."""
...
def normalize_thinking_level(value: str | None) -> ThinkingLevel:
    """Return a valid Xoder thinking level or raise a user-facing error."""
    if value is None:
...
def reasoning_effort_for_level(level: str | None) -> ReasoningEffort:
    """Map Xoder's UI thinking level to an OpenAI-compatible reasoning effort."""
...
def anthropic_thinking_budget_for_level(level: str | None) -> int | None:
    """Map Xoder's UI thinking level to Anthropic extended-thinking tokens."""
    normalized = normalize_thinking_level(level)
    if normalized == "off":
        return None
    return {
