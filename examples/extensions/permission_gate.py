"""Xoder extension that blocks dangerous bash commands before they run.
...
Install by copying into `~/.xoder/extensions/`, or run:

    xoder -x examples/extensions/permission_gate.py
"""

import re

from xoder_coding.extensions import ExtensionAPI, ToolCallHookEvent, ToolCallHookResult
...
def setup(xoder: ExtensionAPI) -> None:
    """Subscribe the bash guard."""
    xoder.on("tool_call", _gate_tool_call)
